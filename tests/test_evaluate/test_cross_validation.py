...
def test_BiCrossValidation_infer_spatial_dynemo():

    save_dir = './test_BiCrossValidation_infer_spatial_dynemo/'
    if os.path.exists(save_dir):
        shutil.rmtree(save_dir)
    os.makedirs(save_dir)
    ...
    with open(f"{save_dir}/alp.pkl", "wb") as f:
        pickle.dump(alpha_truth, f)
    temporal_X_train = {'alpha':f"{save_dir}/alp.pkl"}
    ...
    bcv = BiCrossValidation(config)
    spatial_X_train = bcv.infer_spatial(row_train, column_X,temporal_X_train)
...
def test_BiCrossValidation_calculate_log_likelihood_hmm():

    save_dir = './test_BiCrossValidation_calculate_log_likelihood_hmm/'
    if os.path.exists(save_dir):
        shutil.rmtree(save_dir)
    ...
    # Define spatial and temporal params
    means = np.zeros((3, 2))

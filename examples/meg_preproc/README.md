...
- For task data with correlated bilateral responses (e.g. an auditory task), set `use_bilateral_pairs = True` in `4_source_recon.py` to compute joint beamformer weights for pairs of bilaterally symmetric dipoles. A standard LCMV beamformer partially suppresses correlated sources. A QC plot of the dipole pairs is added to the report ("Beamforming" tab).

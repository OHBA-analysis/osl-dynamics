...
- For task data with correlated bilateral responses (e.g. an auditory task), pass `use_bilateral_pairs=True` to `source_recon.lcmv_beamformer` in `4_source_recon.py` to compute joint beamformer weights for pairs of bilaterally symmetric dipoles (a standard LCMV beamformer partially suppresses correlated sources). A QC plot of the dipole pairs is saved to the session's `src` directory (`bilateral_dipoles.png`). See the [MNE Sample Data: Bilateral Beamformer tutorial](https://osl-dynamics.readthedocs.io/en/latest/tutorials_build/0-5_mne_sample_data.html) for more details.

...
for i in range(n_states):
    tmp = gaussian_heatmap(center = (np.random.randint(1,n_chans-1), np.random.randint(1,n_chans-1)), image_size = (n_chans, n_chans), sig = 5) + np.eye(n_chans,n_chans)
    GT_covz[i,:,:]=tmp@tmp.T
...
prediction_dataset = meg_data.prediction_dataset(config.sequence_length, config.batch_size)

...
class MixedHMMSimulation(Simulation):
    """Simulates a mixture of independent HMMs."""
    ...
    def generate_states(self) -> np.ndarray:
        stc = np.zeros([self.n_samples, self.n_states])
        for n in range(self.n_components):
            ...
            for sample in range(1, self.n_samples):
                states[sample] = next(rands[states[sample - 1]])
            stc += get_one_hot(states, n_states=self.n_states)
        stc /= self.n_components
        return stc

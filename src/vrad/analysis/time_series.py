...
def get_state_time_series(data, state_probabilities):
    ...
    if data.shape[0] != state_probabilities.shape[0]:
        raise ValueError(
            "data and state_probabilities have different lengths:"
            + f"data.shape[0]={data.shape[0]},"
            + f"state_probabilities.shape[0]={state_probabilities.shape[0]}"
        )

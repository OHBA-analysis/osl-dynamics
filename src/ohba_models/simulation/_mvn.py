...
class MultiSubject_MVN(MVN):
    ...
    def __init__(
        ...
        self.subject_means = self._rng.normal(
            loc=self.means,
            scale=between_subject_std,
            size=(self.n_subjects, self.n_modes, self.n_channels),
        )
        self.subject_covariances = self._rng.normal(
            loc=self.covariances,
            scale=between_subject_std,
    ...
    def simulate_multi_subject_data(self, mode_time_courses: np.ndarray) -> np.ndarray:
        ...
        for subject in range(self.n_subjects):
            data.append(self.simulate_data(mode_time_courses[subject]))
        data = np.array(data)
        return data

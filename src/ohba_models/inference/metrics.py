...
def principal_component_distances(matrices, distance_measure):
    ...
    """
    eigvals, eigvecs = np.linalg.eigh(np.abs(matrices))
    principal_eigvecs = np.abs(eigvecs[:, :, -1])
        ...
        for j in range(i + 1, n_matrices):
            if distance_measure == "l2":
                distances[i][j] = np.sum(
                    np.square(principal_eigvecs[i] - principal_eigvecs[j])
                )
            elif distance_measure == "cosine":
                distances[i][j] = cosine(principal_eigvecs[i], principal_eigvecs[j])
            else:
                raise NotImplementedError(distance_measure)
...
def degree_distances(matrices, distance_measure):
    ...
    degrees = np.sum(np.abs(matrices - np.eye(n_channels)), axis=-1)
